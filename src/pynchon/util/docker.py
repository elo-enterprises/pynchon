...
"""
import functools
from pynchon.util import lme,typing,os
...
class DockerCtx():
    ...
    def _run(self, command=None, entrypoint=None,
        working_dir=None, volumes={},
        workspace=False, **invoke_kwargs):
    ...
    def init(self):
        ...
        if self.dockerfile:
            ...
            LOGGER.warning(f"i have tag {self.tag}, but can't be sure it's built or pulled")
            return False
        else:
    ...
    def init_dockerfile(self):
        ...
        if not result:
            ...
            cmd = os.invoke(
                f"docker build --tag '{self.tag}' . -f {self.file}")
        else:
    ...
    def __call__(self, command=None, entrypoint=None, script=None, **docker_or_invoke_kwargs):
        """
...
Dockerfile=lambda txt, **kwargs: functools.partial(DockerCtx, dockerfile=txt, **kwargs)()

if __name__=='__main__':
    result = Dockerfile('''FROM python:3.8-alpine''').run(script='printf "{}"', load_json=True)
    result.json
