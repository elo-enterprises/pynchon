...
class PluginsMan(models.Provider):
    ...
    def schema(self, plugin_name, markdown:bool=False, json_schema:bool=True) -> str:
        ...
        if json_schema:
            ...
            t = render.get_template(from_string="""\n{%for field in fields%}\n**{{plugin_name}}.{{field}}:** `{{fields[field]}}`{%endfor%}\n""",template_name='asdasdsa')
            fields = dict([[k,v] for k,v in plugin.config.__class__.__fields__.items()])
            fields = dict([[k, str(v).replace(f"name='{k}'",'').lstrip()] for k,v in fields.items()])
